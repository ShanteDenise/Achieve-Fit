...
class Server extends EventEmitter {
   ...
   * Create a server
   *
   * @param {ServerDescription***REMOVED*** description
  ...
  ***REMOVED***
   ...
   *
   * @param {Array***REMOVED*** [options.auth] Array of auth options to apply on connect
   */
  connect(options) {
    ...
    if (this.s.pool && !this.s.pool.isDisconnected() && !this.s.pool.isDestroyed()) {
      throw new MongoError(`Server instance in invalid state ${this.s.pool.state***REMOVED***`);
    ***REMOVED***

    // create a pool
    this.s.pool = new Pool(this, Object.assign(this.s.options, options, { bson: this.s.bson ***REMOVED***));
  ...
  command(ns, cmd, options, callback) {
    ...
    ***REMOVED***
    ...
    if (this.s.logger.isDebug()) {
      this.s.logger.debug(
        `executing command [${JSON.stringify({ ns, cmd, options ***REMOVED***)***REMOVED***] against ${this.name***REMOVED***`
      );
    ***REMOVED***

    // Check if we have collation support
    if (this.description.maxWireVersion < 5 && cmd.collation) {
      callback(new MongoError(`server ${this.name***REMOVED*** does not support collation`));
      return;
    ***REMOVED***

    // Are we executing against a specific topology
    const topology = options.topology || {***REMOVED***;
    // Create the query object
    const query = this.s.wireProtocolHandler.command(this.s.bson, ns, cmd, {***REMOVED***, topology, options);
  ...
  ***REMOVED***
  ...
  insert(ns, ops, options, callback) {
    executeWriteOperation({ server: this, op: 'insert', ns, ops ***REMOVED***, options, callback);
  ***REMOVED***
  ...
  update(ns, ops, options, callback) {
    executeWriteOperation({ server: this, op: 'update', ns, ops ***REMOVED***, options, callback);
  ***REMOVED***
  ...
  remove(ns, ops, options, callback) {
    executeWriteOperation({ server: this, op: 'remove', ns, ops ***REMOVED***, options, callback);
  ***REMOVED***
***REMOVED***
...
function executeServerHandshake(server, callback) {
  ...
  const queryOptions = { numberToSkip: 0, numberToReturn: -1, checkKeys: false, slaveOk: true ***REMOVED***;
  const query = new Query(
    server.s.bson,
    'admin.$cmd',
    Object.assign(
      { ismaster: true, client: server.s.clientInfo, compression: compressors ***REMOVED***,
  ...
  server.s.pool.write(
    query,
    { socketTimeout: server.s.options.connectionTimeout || 2000 ***REMOVED***,
    callback
  );
***REMOVED***
...
function connectEventHandler(server) {
  return function() {
    ...
    //   server.s.logger.info(`server ${server.name***REMOVED*** fired event ${event***REMOVED*** out with message ${object***REMOVED***`);
    // ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (server.s.logger.isInfo()) {
        server.s.logger.info(
          `server ${server.name***REMOVED*** connected with ismaster [${JSON.stringify(isMaster)***REMOVED***]`
        );
      ***REMOVED***
