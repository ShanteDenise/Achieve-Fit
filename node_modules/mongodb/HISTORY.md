...
* **collection:** allow { upsert: 1 ***REMOVED*** for findOneAndUpdate() and update() ([5bcedd6](https://github.com/mongodb/node-mongodb-native/commit/5bcedd6))
...
* NODE-430 Cursor.count() opts argument masked by var opts = {***REMOVED***
...
* Implementing read preference setting at collection and cursor level
   ...
   * db.collection("some", {readPreference:Server.SECONDARY***REMOVED***)
...
* Allow for force send query to primary, pass option (read:'primary') on find command.
    * ``find({a:1***REMOVED***, {read:'primary'***REMOVED***).toArray(function(err, items) {***REMOVED***);``
...
* Eval function no accepts options including nolock (Issue #432)
    * eval(code, parameters, options, callback) (where options = {nolock:true***REMOVED***)
...
* FindOne and find treats selector == null as {***REMOVED***, Issue #403
...
* Added logger option for Db class, lets you pass in a function in the shape
    {
        log : function(message, object) {***REMOVED***,
        error : function(errorMessage, errorObject) {***REMOVED***,
        debug : function(debugMessage, object) {***REMOVED***,
    ***REMOVED***

  Usage is new Db(new Server(..), {logger: loggerInstance***REMOVED***)
...
* Allows for forcing the server to assign ObjectID's using the option {forceServerObjectId: true***REMOVED***
