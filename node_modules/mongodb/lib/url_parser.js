  ...
  dns.resolveSrv(srvAddress, function(err, addresses) {
    ...
    ***REMOVED***
    ...
    let connectionStrings = addresses.map(function(address, i) {
      if (i === 0) return `${base***REMOVED***${address.name***REMOVED***:${address.port***REMOVED***`;
      else return `${address.name***REMOVED***:${address.port***REMOVED***`;
    ***REMOVED***);
    ...
    ***REMOVED*** else if (!options.ssl && result.search && !result.search.match('ssl')) {
      connectionStringOptions.push('ssl=true');
    ***REMOVED***
...
function matchesParentDomain(srvAddress, parentDomain) {
  let regex = /^.*?\./;
  let srv = `.${srvAddress.replace(regex, '')***REMOVED***`;
  let parent = `.${parentDomain.replace(regex, '')***REMOVED***`;
...
function parseConnectionString(url, options) {
  ...
  ***REMOVED***
  ...
  for (let i = 0; i < strings.length; i++) {
    ...
    if (hostString.indexOf('mongodb') !== -1) {
      ...
      ***REMOVED***
    ***REMOVED*** else if (hostString.indexOf('/') !== -1) {
  ...
  ***REMOVED***
  ...
  if (connection_part.indexOf('.sock') !== -1) {
    ...
    ***REMOVED***
  ***REMOVED*** else if (connection_part.indexOf('/') !== -1) {
    // Check if multiple database names provided, or just an illegal trailing backslash
    if (connection_part.split('/').length > 2) {
  ...
  ***REMOVED***
  ...
  if (auth.length === 2) object.auth = { user: auth[0], password: auth[1] ***REMOVED***;
  ...
  let servers;
  let compression;
  let serverOptions = { socketOptions: {***REMOVED*** ***REMOVED***;
  let dbOptions = { read_preference_tags: [] ***REMOVED***;
  let replSetServersOptions = { socketOptions: {***REMOVED*** ***REMOVED***;
  let mongosOptions = { socketOptions: {***REMOVED*** ***REMOVED***;
  ...
  if (url.match(/\.sock/)) {
    ...
    servers = [{ domain_socket: domainSocket ***REMOVED***];
  ***REMOVED***
    // Split up the db
    hostPart = connection_part;
    // Deduplicate servers
    let deduplicatedServers = {***REMOVED***;
    ...
    servers = hostPart
      ...
      .map(function(h) {
        ...
        ***REMOVED***
        ...
        return { host: _host, port: _port ***REMOVED***;
      ***REMOVED***)
      .filter(function(x) {
        return x != null;
      ***REMOVED***);
  ...
  urlOptions.forEach(function(opt) {
    ...
    switch (name) {
      ...
      case 'readConcernLevel':
        dbOptions.readConcern = { level: value ***REMOVED***;
        break;
      ...
      case 'authMechanism':
        if (value === 'GSSAPI') {
          ...
          if (object.auth == null) {
            ...
            object.auth = { user: urlDecodeAuthPart, password: null ***REMOVED***;
          ***REMOVED***
            object.auth.user = decodeURIComponent(object.auth.user);
          ***REMOVED***
        ***REMOVED*** else if (value === 'MONGODB-X509') {
      ...
      case 'compressors':
        {
          compression = serverOptions.compression || {***REMOVED***;
      ...
      default:
        {
          ...
          logger.warn(`${name***REMOVED*** is not supported as a connection string option`);
        ***REMOVED***
        break;
    ***REMOVED***
  ***REMOVED***);
