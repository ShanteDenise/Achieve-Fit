...
schema.pre('find', () => { console.log('find!'); ***REMOVED***);
...
mongoose.connect('mongodb://localhost:27017/test').model('Test', new Schema({***REMOVED***));

// Do this instead
mongoose.connect('mongodb://localhost:27017/test');
mongoose.model('Test', new Schema({***REMOVED***));
```
...
await BlogPost.create({ title: 'test', author: author._id ***REMOVED***);
...
// cursor.
const cursor = MyModel.aggregate([{ $match: { name: 'Val' ***REMOVED*** ***REMOVED***]).cursor().exec();
...
const cursorWithOptions = MyModel.
  aggregate([{ $match: { name: 'Val' ***REMOVED*** ***REMOVED***]).
  cursor({ batchSize: 10 ***REMOVED***).
  exec();
```
...
mongoose.createConnection(`mongodb://aaron:psw@${host***REMOVED***/fake`);
...
const schema = new Schema({ name: String ***REMOVED***, { toObject: { virtuals: true ***REMOVED*** ***REMOVED***);
schema.virtual('answer').get(() => 42);
const MyModel = db.model('MyModel', schema);

const doc = new MyModel({ name: 'test' ***REMOVED***);
// In mongoose 4.x this prints "undefined", because `{ minimize: false ***REMOVED***`
// overwrites the entire schema-defined options object.
// In mongoose 5.x this prints "42", because `{ minimize: false ***REMOVED***` gets
// merged with the schema-defined options.
console.log(doc.toJSON({ minimize: false ***REMOVED***).answer);
```
...
```javascript
MyModel.aggregate({ $match: { isDeleted: false ***REMOVED*** ***REMOVED***, { $skip: 10 ***REMOVED***).exec(cb);
```
...
MyModel.create({ boolField: 'not a boolean' ***REMOVED***);
```
...
```javascript
// In mongoose 4.x, this becomes `{ $set: { name: 'Baz' ***REMOVED*** ***REMOVED***` despite the `overwrite`
// In mongoose 5.x, this overwrite is respected and the first document with
// `name = 'Bar'` will be replaced with `{ name: 'Baz' ***REMOVED***`
User.where({ name: 'Bar' ***REMOVED***).update({ name: 'Baz' ***REMOVED***).setOptions({ overwrite: true ***REMOVED***);
```
...
const m = new Model({ children: [{ text: 'test' ***REMOVED***] ***REMOVED***);
m.save(function() {
...
const schema = new Schema({
  email: { type: String, lowercase: true ***REMOVED***
***REMOVED***);
const Model = mongoose.model('Test', schema);
Model.find({ email: 'FOO@BAR.BAZ' ***REMOVED***); // Converted to `find({ email: 'foo@bar.baz' ***REMOVED***)`
```
...
In Mongoose 4.x, overwriting a filter property that's a primitive with one that is an object would silently fail. For example, the below code would ignore the `where()` and be equivalent to `Sport.find({ name: 'baseball' ***REMOVED***)`

```javascript
Sport.find({ name: 'baseball' ***REMOVED***).where({name: {$ne: 'softball'***REMOVED******REMOVED***);
```

In Mongoose 5.x, the above code will correctly overwrite `'baseball'` with `{ $ne: 'softball' ***REMOVED***` 
...
```javascript
const Model = mongoose.model('Test', new Schema({ name: String ***REMOVED***));

const res = await Model.bulkWrite([{ insertOne: { document: { name: 'test' ***REMOVED*** ***REMOVED*** ***REMOVED***]);
...
BulkWriteResult {
  ...
  upsertedCount: 0,
  upsertedIds: {***REMOVED***,
  insertedIds: { '0': 5be9a3101638a066702a0d38 ***REMOVED***,
  n: 1 ***REMOVED***
```
...
BulkWriteResult {
  result: 
   { ok: 1,
     ...
     nRemoved: 0,
     upserted: [],
     lastOp: { ts: [Object], t: 1 ***REMOVED*** ***REMOVED***,
  insertedCount: 1,
  matchedCount: 0,
  modifiedCount: 0,
  deletedCount: 0,
  upsertedCount: 0,
  upsertedIds: {***REMOVED***,
  insertedIds: { '0': 5be9a1c87decfc6443dd9f18 ***REMOVED***,
  n: 1 ***REMOVED***
```