  ...
  ***REMOVED***;
  ...
  if (!options || !options[defaultMongooseSymbol]) {
    ...
    ***REMOVED***;
    ...
    this.Schema.Types = Object.assign({***REMOVED***, Schema.Types);
  ***REMOVED***
  ...
  Object.defineProperty(this, 'plugins', {
    ...
    writable: false,
    value: [
      [saveSubdocs, { deduplicate: true ***REMOVED***],
      [validateBeforeSave, { deduplicate: true ***REMOVED***],
      [shardingPlugin, { deduplicate: true ***REMOVED***],
      [removeSubdocs, { deduplicate: true ***REMOVED***]
    ]
  ***REMOVED***);
***REMOVED***
 ...
 *
 *     mongoose.set('debug', function(collectionName, methodName, arg1, arg2...) {***REMOVED***); // use custom function to log collection methods + arguments
 *
 ...
 * - 'toObject': `{ transform: true, flattenDecimals: true ***REMOVED***` by default. Overwrites default objects to [`toObject()`](/docs/api.html#document_Document-toObject)
 * - 'toJSON': `{ transform: true, flattenDecimals: true ***REMOVED***` by default. Overwrites default objects to [`toJSON()`](/docs/api.html#document_Document-toJSON), for determining how Mongoose documents get serialized by `JSON.stringify()`
 * - 'strict': true by default, may be `false`, `true`, or `'throw'`. Sets the default strict mode for schemas.
 * - 'selectPopulatedPaths': true by default. Set to false to opt out of Mongoose adding all fields that you `populate()` to your `select()`. The schema-level option `selectPopulatedPaths` overwrites this one.
 *
 * @param {String***REMOVED*** key
 * @param {String|Function|Boolean***REMOVED*** value
 * @api public
 */
 ...
 *
 *     // and options
 *     var opts = { db: { native_parser: true ***REMOVED******REMOVED***
 ...
 *
 *     // and options
 *     var opts = { replset: { strategy: 'ping', rs_name: 'testSet' ***REMOVED******REMOVED***
 *     db = mongoose.createConnection('mongodb://user:pass@localhost:port,anotherhost:port,yetanother:port/database', opts);
 *
 *     // and options
 *     var opts = { server: { auto_reconnect: false ***REMOVED***, user: 'username', pass: 'mypassword' ***REMOVED***
 ...
 *
 * @param {String***REMOVED*** [uri] a mongodb:// URI
 * @param {Object***REMOVED*** [options] passed down to the [MongoDB driver's `connect()` function](http://mongodb.github.io/node-mongodb-native/3.0/api/MongoClient.html), except for 4 mongoose-specific options explained below.
 * @param {String***REMOVED*** [options.user] username for authentication, equivalent to `options.auth.user`. Maintained for backwards compatibility.
 * @param {String***REMOVED*** [options.pass] password for authentication, equivalent to `options.auth.password`. Maintained for backwards compatibility.
 * @param {Boolean***REMOVED*** [options.autoIndex=true] Mongoose-specific option. Set to false to disable automatic index creation for all models associated with this connection.
 * @param {Boolean***REMOVED*** [options.bufferCommands=true] Mongoose specific option. Set to false to [disable buffering](http://mongoosejs.com/docs/faq.html#callback_never_executes) on all models associated with this connection.
 * @return {Connection***REMOVED*** the created Connection object. Connections are thenable, so you can do `await mongoose.createConnection()`
 * @api public
 */
 ...
 *     ***REMOVED***)
 *
 * @param {String***REMOVED*** uri(s)
 * @param {Object***REMOVED*** [options] passed down to the [MongoDB driver's `connect()` function](http://mongodb.github.io/node-mongodb-native/3.0/api/MongoClient.html), except for 4 mongoose-specific options explained below.
 * @param {String***REMOVED*** [options.dbName] The name of the database we want to use. If not provided, use database name from connection string.
 * @param {String***REMOVED*** [options.user] username for authentication, equivalent to `options.auth.user`. Maintained for backwards compatibility.
 * @param {String***REMOVED*** [options.pass] password for authentication, equivalent to `options.auth.password`. Maintained for backwards compatibility.
 * @param {Boolean***REMOVED*** [options.autoIndex=true] Mongoose-specific option. Set to false to disable automatic index creation for all models associated with this connection.
 * @param {Boolean***REMOVED*** [options.bufferCommands=true] Mongoose specific option. Set to false to [disable buffering](http://mongoosejs.com/docs/faq.html#callback_never_executes) on all models associated with this connection.
 * @param {Boolean***REMOVED*** [options.useCreateIndex=true] Mongoose-specific option. If `true`, this connection will use [`createIndex()` instead of `ensureIndex()`](/docs/deprecations.html#-ensureindex-) for automatic index builds via [`Model.init()`](/docs/api.html#model_Model.init).
 * @param {Boolean***REMOVED*** [options.useFindAndModify=true] True by default. Set to `false` to make `findOneAndUpdate()` and `findOneAndRemove()` use native `findOneAndUpdate()` rather than `findAndModify()`.
 * @param {Boolean***REMOVED*** [options.useNewUrlParser=false] False by default. Set to `true` to make all connections set the `useNewUrlParser` option by default.
 * @param {Function***REMOVED*** [callback]
 * @see Mongoose#createConnection #index_Mongoose-createConnection
 * @api public
 * @return {Promise***REMOVED*** resolves to `this` if connection succeeded
 */
 ...
 *
 * @param {Function***REMOVED*** [callback] called after all connection close, or when first error occurred.
 * @return {Promise***REMOVED*** resolves when all connections are closed, or rejects with the first error that occurred.
 * @api public
 */
 ...
 *
 * @param {Function|null***REMOVED*** [fn] overwrites the function used to pluralize collection names
 * @return {Function|null***REMOVED*** the current function used to pluralize collection names, defaults to the legacy function from `mongoose-legacy-pluralize`.
 * @api public
 */
 ...
 *     mongoose.model('Actor', new Schema({ name: String ***REMOVED***));
 *
 ...
 *
 * @param {String|Function***REMOVED*** name model name or class extending Model
 * @param {Schema***REMOVED*** [schema]
 * @param {String***REMOVED*** [collection] name (optional, inferred from model name)
 * @param {Boolean***REMOVED*** [skipInit] whether to skip initialization (defaults to false)
 ...
 * ####Example:
 *
 *     mongoose.model('User', new Schema({ name: String ***REMOVED***));
 ...
 *
 * @param {Function***REMOVED*** fn plugin callback
 ...
 * ####Example:
 *
 *     const childSchema = new Schema({ parentId: mongoose.ObjectId ***REMOVED***);
 *
 * @property ObjectId
 * @api public
 */
 ...
 * ####Example:
 *
 *     const vehicleSchema = new Schema({ fuelLevel: mongoose.Decimal128 ***REMOVED***);
 *
 * @property Decimal128
 * @api public
 */
 ...
 * ####Example:
 *
 *     const schema = new Schema({ arbitrary: mongoose.Mixed ***REMOVED***);
 *
 * @property Mixed
 * @api public
 */
...
Mongoose.prototype.now = function now() { return new Date(); ***REMOVED***;

/**
 * The Mongoose CastError constructor
 *
 * @method CastError
 * @param {String***REMOVED*** type The name of the type
 * @param {Any***REMOVED*** value The value that failed to cast
 * @param {String***REMOVED*** path The path `a.b.c` in the doc where this cast error occurred
 * @param {Error***REMOVED*** [reason] The original error that was thrown
 * @api public
 */
