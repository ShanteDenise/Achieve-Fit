...
require('mongodb').connect(uri, function (err, db) {
  ...
  mquery(collection).find({..***REMOVED***, callback);

  // or pass it to the collection method
  mquery().find({..***REMOVED***).collection(collection).exec(callback)
...
mquery().remove(match, function (err){***REMOVED***)
```
...
mquery().update(match, updateDocument, options, function (err, result){***REMOVED***)
...
```js
mquery(collection).where({ _id: id ***REMOVED***).update({ title: 'words' ***REMOVED***, callback)
```

becomes

```js
collection.update({ _id: id ***REMOVED***, { $set: { title: 'words' ***REMOVED******REMOVED***, callback)
```
...
Passing an empty object `{ ***REMOVED***` as the update document will result in a no-op unless the `overwrite` option is passed. Without the `overwrite` option, the update operation will be ignored and the callback executed without sending the command to MongoDB to prevent accidently overwritting documents in the collection.

```js
var q = mquery(collection).where({ _id: id ***REMOVED***).setOptions({ overwrite: true ***REMOVED***);
q.update({ ***REMOVED***, callback); // overwrite with an empty doc
```
...
```js
// create a base query
var base = mquery({ _id: 108 ***REMOVED***).collection(collection).toConstructor();

base().findOne(function (err, doc) {
  console.log(doc); // { _id: 108, name: 'cajon' ***REMOVED***)

  base().setOptions({ overwrite: true ***REMOVED***).update({ changed: true ***REMOVED***, function (err) {
    base.findOne(function (err, doc) {
      console.log(doc); // { _id: 108, changed: true ***REMOVED***) - the doc was overwritten
    ***REMOVED***);
  ***REMOVED***);
***REMOVED***)
```
...
mquery()
  .collection(coll)
  .update({ name: /^match/ ***REMOVED***, { $addToSet: { arr: 4 ***REMOVED******REMOVED***, { multi: true ***REMOVED***, callback)

// another way of doing it
mquery({ name: /^match/ ***REMOVED***)
  .collection(coll)
  .setOptions({ multi: true ***REMOVED***)
  .update({ $addToSet: { arr: 4 ***REMOVED******REMOVED***, callback)

// update multiple documents with an empty doc
var q = mquery(collection).where({ name: /^match/ ***REMOVED***);
...
```js
mquery().findOne().where('route').intersects(polygon).exec(function (err, docs){***REMOVED***)
```
...
```js
mquery().and([{ color: 'green' ***REMOVED***, { status: 'ok' ***REMOVED***])
```
...
// or
query.where('loc').near().geometry({ type: 'Point', coordinates: [3,5] ***REMOVED***)
```
...
```js
mquery().where('location').near({ center: [139, 74.3] ***REMOVED***).maxDistance(5)
```
...
```js
mquery().nor([{ color: 'green' ***REMOVED***, { status: 'ok' ***REMOVED***])
```
...
// GeoJSON
query.where('loc').near({ center: { type: 'Point', coordinates: [10, 10] ***REMOVED******REMOVED***);
query.where('loc').near({ center: { type: 'Point', coordinates: [10, 10] ***REMOVED***, maxDistance: 5, spherical: true ***REMOVED***);
query.where('loc').near().geometry({ type: 'Point', coordinates: [10, 10] ***REMOVED***);

// For a $nearSphere condition, pass the `spherical` option.
query.near({ center: [10, 10], maxDistance: 5, spherical: true ***REMOVED***);
```
...
```js
mquery().or([{ color: 'red' ***REMOVED***, { status: 'emergency' ***REMOVED***])
```
...
mquery().select({ name: 1, address: 1, _id: 0 ***REMOVED***)
...
mquery().where('loc').within({ center: [50,50], radius: 10, unique: true, spherical: true ***REMOVED***);
mquery().where('loc').within({ box: [[40.73, -73.9], [40.7, -73.988]] ***REMOVED***);
mquery().where('loc').within({ polygon: [[],[],[],[]] ***REMOVED***);

mquery().where('loc').within([], [], []) // polygon
mquery().where('loc').within([], []) // box
mquery().where('loc').within({ type: 'LineString', coordinates: [...] ***REMOVED***); // geometry
```
...
```js
// instead of writing:
mquery().find({age: {$gte: 21, $lte: 65***REMOVED******REMOVED***);

// we can instead write:
mquery().where('age').gte(21).lte(65);

// passing query conditions is permitted too
mquery().find().where({ name: 'vonderful' ***REMOVED***)

// chaining
mquery()
.where('age').gte(21).lte(65)
.where({ 'name': /^vonderful/i ***REMOVED***)
...
```js
query.collation({ locale: "en_US", strength: 1 ***REMOVED***)
```
...
```js
mquery().hint({ indexA: 1, indexB: -1 ***REMOVED***)
```
...
var preference = new ReadPref('secondary', [{ dc:'sf', s: 1 ***REMOVED***,{ dc:'ma', s: 2 ***REMOVED***]);
...
writeConcern({ w: `<value>`, j: `<boolean>`, wtimeout: `<number>` ***REMOVED***`)
...
```js
co(function*(){
  var doc = yield mquery().findOne({ _id: 499 ***REMOVED***);
  console.log(doc); // { _id: 499, name: 'amazing', .. ***REMOVED***
***REMOVED***)();
```
...
```js
var thunk = mquery(collection).find({..***REMOVED***).thunk();
...
```js
var drum = mquery({ type: 'drum' ***REMOVED***).collection(instruments);
var redDrum = mquery({ color: 'red' ***REMOVED***).merge(drum);
...
```js
mquery().setOptions({ collection: coll, limit: 20 ***REMOVED***)
```
...
- [hint](#hint) *
- [slaveOk](#slaveOk) *
- [safe](http://docs.mongodb.org/manual/reference/write-concern/): Boolean - passed through to the collection. Setting to `true` is equivalent to `{ w: 1 ***REMOVED***`
...
mquery().setTraceFunction(traceFunction).findOne({name: 'Joe'***REMOVED***, cb);
```
...
```js
var query = mquery({ type: 'drum' ***REMOVED***);
...
greatMovies().where({ name: /^Life/ ***REMOVED***).select('name').find(function (err, docs) {
  console.log(docs);
***REMOVED***);
```
